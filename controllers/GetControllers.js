...
module.exports = {
    ...
    getFromAPI: (req, res) => {
        axios.get(`https://api.tutorah.tv/v1${req.body.endpoint}`, { headers: { "Authorization": req.body.key } })
            .then((response) => {
                let result = response.data

                res.status(200).json({ message: 'getFromAPI loaded!', data: result })
            })
            .catch((err) => { console.log(err); res.status(400).json({ message: 'I am Error / getFromAPI failed :(', data: err }) })
    }
}